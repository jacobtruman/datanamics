...
$page_items = new PageItems();


$root = dirname(__FILE__);
//require_once("{$root}/session.php");
require_once("{$root}/styles.php");
require_once("{$root}/searchheader.php");

$search_params = array("name", "shtype", "pcode", "gip", "rtr", "addr", "special", "phone", "gtype");
...
if(($name == '*') || ($special == '*')) {
	...
	$sql = "SELECT * FROM property WHERE billing_method LIKE '%Eleven Wireless%' ORDER BY {$order}";
	$y = 6;
} else {
	if($phone != '..') {
		$sql = "SELECT * FROM {$table} AS p INNER JOIN contact AS c ON p.prop = c.prop WHERE (phone LIKE '%{$phone}%') ORDER BY p.property_type_id, p.name";
	} else if($addr != '..') {
		$sql = "SELECT * FROM {$table} AS p INNER JOIN contact AS c ON p.prop = c.prop WHERE (staddr LIKE '%{$addr}%') ORDER BY p.property_type_id, p.name";
	} else if(($shtype != '..') && ($name != '..') && ($pcode != '..') && ($billing != '..')) {
		$sql = "SELECT * FROM {$table} WHERE (property_type_id = '{$shtype}') AND (name LIKE '%{$name}%') AND (prop LIKE '%{$pcode}%') AND (prop IN (SELECT prop FROM devices WHERE ip LIKE '%{$gip}%')) AND (rtr LIKE '%{$rtr}%') ORDER BY {$order}";
	} else if(($shtype != '..') && ($name != '..')) {
		$sql = "SELECT * FROM {$table} WHERE (property_type_id = '{$shtype}') AND ((name LIKE '%{$name}%') OR (property_type_id LIKE '%{$name}%')) ORDER BY {$order}";
	} else {
		$sql = "SELECT * FROM {$table} WHERE (property_type_id = '{$shtype}') OR ((name LIKE '%{$name}%') OR (property_type_id LIKE '%{$name}%')) OR (prop LIKE '%{$pcode}%') OR (prop IN (SELECT prop FROM devices WHERE ip LIKE '%{$gip}%' OR ip LIKE '%{$rtr}%')) ORDER BY {$order}";
	}
}

if($y == 3) {
	$sql = "SELECT * FROM {$table} WHERE (SELECT prop FROM devices WHERE type LIKE '%{$gtype}%') ORDER BY property_type_id, name";
	$form = "ResultsGateway.php";
} else if($y == 6) {
	$sql = "SELECT * FROM {$table} WHERE poc = 'No' AND billing_method LIKE '%Eleven Wireless%' ORDER BY {$order}";
	$form = "Results11OS.php";
} else {
	$form = "results_body.php";
}
