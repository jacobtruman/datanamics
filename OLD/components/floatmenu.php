<?php include($root."/support_web/components/gLinkGen.php");?>
<form name='contact' method='POST' action='/support_web/forms/ContactInformation.php' target='_blank'><input type='hidden' name='prop' value='<? echo $prop;?>'></form>
<form name='overview' method='POST' action='<?php echo $pover;?>' target='_blank'><input type='hidden' name='prop' value='<?php echo $prop;?>'></form>
<form name='ipinfo' method='POST' action='<?php echo $ipinfo;?>' target='_blank'><input type='hidden' name='prop' value='<?php echo $prop;?>'></form>
<?php
	if ((!empty($notes)) || (!empty($notes_level2))){
		echo "<form name='notes' method='POST' action='".$notespage."' target='_blank'>\n";
			echo "<input type='hidden' name='prop' value='".$prop."'>\n";
			echo "<input type='hidden' name='lvl' value='".$lvl."'>\n";
		echo "</form>\n";
	}
?>
	<form name='custinfo' method='POST' action='file:///Z:/<?php echo $fold;?>' target='_blank'></form>
	<form name='lvl2' method='POST' action='/support_web/2nd Level Support/forms/passwords.php' target='_blank'></form>
...
var hBar = new ItemStyle(17, 10, '', 0, 0, '#000000', '#858585', 'highText', 'highText', '', '',
 null, null, 'hand', 'default');

var subM = new ItemStyle(22, 0, '&gt;', -15, 3, '#000000', '#858585', 'lowText', 'highText',
 '', '', null, null, 'hand', 'default');

var subBlank = new ItemStyle(22, 1, '&gt;', -15, 3, '#858585', '#000000', 'lowText', 'highText',
 '', '', null, null, 'hand', 'default');
...
with (pMenu)
{
startMenu('root', true, 4, 'window.page.scrollY()+13', 70, hBar);
addItem('�Menu', 'mSupport', 'sm:');
startMenu('mSupport', true, 70, 0, 180, subM);
addItem('Property Contact','document.forms.contact.submit()','js:');
addItem('Property Overview','document.forms.overview.submit()','js:');
addItem('IP Information','document.forms.ipinfo.submit()','js:');
<?php
include("getPropPrefix.php");

if (($lvl == '2') && ($pnagios == 'Yes')){
	echo "addItem('Nagios Status', 'window.open(";
	echo '"';
	echo "http://datadmin:zinck2240@12.110.36.139/nagios/cgi-bin/status.cgi?hostgroup=".$propPrefix."-".$prop."&style=detail";
	echo '"';
	echo ")' , 'js:');\n";
}

if ((!empty($notes)) || (!empty($notes_level2))){
		echo "addItem('Notes','document.forms.notes.submit()','js:');";
	}else{
		echo "addItem('No Notes','','');";
	}

echo "addItem('Customer Information','document.forms.custinfo.submit()','js:');";

if ($lvl == '2'){
	echo "addItem('Equipment Passwords','document.forms.lvl2.submit()','js:');";
}
...
if(!empty($gateway)){
	for($n = 1; $n <= 4; $n++){
		...
		if(!empty($$gn[$n])){
			echo "addItem('Gateway ".$n." Web Interface', 'window.open(";
			echo '"';
			echo $$ncStr;
			echo '"';
			echo ")' , 'js:');";
			echo "addItem('Gateway ".$n." Telnet', 'telnet:".$$gn[$n]."' , '');";
		}
	}
}
